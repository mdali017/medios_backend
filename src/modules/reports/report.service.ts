...
export interface RecentOrderReport {
  id: string
  orderNumber: string
  orderType: string
  status: string
...
export interface StockAlertProduct {
  ...
  expiryDate: string
  status: 'Low Stock' | 'Stock Out' | 'Expiring Soon'
}
...
function isRevenueOrder(row: Record<string, unknown>): boolean {
...
function getStockStatus(
  stockQuantity: number,
  expiryDate: string
): 'In Stock' | 'Low Stock' | 'Stock Out' | 'Expiring Soon' {
...
function resolveReportDateRange(startDate?: string, endDate?: string): {
  start: Date
  end: Date
  startDate: string
  endDate: string
...
function buildDailySalesForRange(
  ...
  for (const order of orders) {
    ...
    if (!isOrderInRange(order.created_at as string, start, end)) continue

    const key = toDateKey(new Date(order.created_at as string))
...
export async function getStoreReports(
  ...
  for (const order of revenueOrders) {
    ...
    totalRevenue += amount
    if (order.order_type === 'pos') posOrders += 1
    if (order.order_type === 'online') onlineOrders += 1
  }

  const averageDailyRevenue =
    dateRange.periodDays > 0
  ...
  for (const product of productRows) {
    ...
    const status = getStockStatus(stockQuantity, product.expiry_date as string)

    if (status === 'In Stock') inStock += 1
    if (status === 'Low Stock') lowStock += 1
    if (status === 'Stock Out') outOfStock += 1
    ...
    if (status !== 'In Stock') {
      alerts.push({
        ...
        stockQuantity,
        expiryDate: product.expiry_date as string,
        status,
      })
    }
  }
  ...
  const revenueOrderIds = revenueOrders.map((order) => order.id as string)
  ...
  const productAgg = new Map<
    string,
    { productName: string; quantitySold: number; revenue: number; estimatedProfit: number | null; hasCost: boolean }
  >()
  ...
  const marginPercent =
    costCoverageRevenue > 0 ? Number(((estimatedProfit / costCoverageRevenue) * 100).toFixed(1)) : null

  const topProducts: TopProductReport[] = Array.from(productAgg.entries())
    .map(([productId, agg]) => ({
      productId,
