...
const storeManagerService = __importStar(require("./store-manager.service"));
async function listStoreManagers(req, res, next) {
    try {
...
async function createStoreManager(req, res, next) {
    try {
        const data = await storeManagerService.createStoreManager(req.user, req.body);
        return (0, apiResponse_1.sendSuccess)(res, data, 'Store manager created successfully', 201);
    }
    catch (error) {
        next(error);
    }
